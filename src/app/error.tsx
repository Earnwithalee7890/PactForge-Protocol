  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, paddingTop: 96 }}>
      <div className="glass-card" style={{ padding: 60, textAlign: "center", maxWidth: 600, width: "100%", border: "1px solid rgba(239,68,68,0.3)" }}>
        ...
        <p style={{ color: "#94a3b8", marginBottom: 20, fontSize: 16 }}>An unexpected protocol error occurred. This could be due to a network glitch or a misconfigured transaction. Please try again or return to your dashboard.</p>
        <div style={{ background: "rgba(0,0,0,0.3)", padding: 16, borderRadius: 8, marginBottom: 40, textAlign: "left", overflowX: "auto" }}>
          <div style={{ color: "#ef4444", fontWeight: "bold", marginBottom: 8 }}>Debug Info:</div>
          <pre style={{ fontSize: 12, color: "#cbd5e1", whiteSpace: "pre-wrap" }}>{error.message}</pre>
          {error.stack && <pre style={{ fontSize: 10, color: "#64748b", marginTop: 8, whiteSpace: "pre-wrap" }}>{error.stack}</pre>}
        </div>
